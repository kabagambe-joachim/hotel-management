...
export const createBooking = async ({
  ...
  const { data } = await axios.post(
    `https://${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/v2023-12-11/data/mutate/${process.env.NEXT_PUBLIC_SANITY_DATASET}`,
    mutation,
...
export const updateHotelRoom = async (hotelRoomId: string) => {
  ...
  const { data } = await axios.post(
    `https://${process.env.NEXT_PUBLIC_SANITY_PROJECT_ID}.api.sanity.io/2023-12-11/data/mutate/${process.env.NEXT_PUBLIC_SANITY_DATASET}`,
    mutation,
    { headers: { Authorization: `Bearer ${process.env.SANITY_STUDIO_TOKEN}` } }
  );
